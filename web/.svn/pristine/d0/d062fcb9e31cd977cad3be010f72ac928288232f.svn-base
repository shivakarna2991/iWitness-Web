...
class iWitness_Access_Token
{
    ...
    protected function parse_from($options)
    {
        ...
        isset($options['expires_in']) and $this->expires_in = (int)$options['expires_in'];
        ...
        $this->created = isset($options['created']) ? (int)$options['created'] : time();
    }
    ...
    {
        return (time() > ($this->created + $this->expires_in - 10));
    }
