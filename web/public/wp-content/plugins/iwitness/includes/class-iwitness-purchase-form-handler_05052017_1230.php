...
class iWitness_Form_Purchase_Handler
{
    ...
    public function  iwitness_do_place_your_order()
    {
        try {
			...
			//$existingUser = iwitness_get_post_field_value('sender_email_hd');
			...
			if ($plan == 'giftplanyear' || $plan == 'freegiftcard') {
                if (empty($recipients)) {
            ...
            if (iwitness_notice_count('error') == 0) {
                ...
                //$session['subscriptionexpdate'] = $response['expire_at'];
                $session['originalPhone'] = $phone;

                //buy gift cards
                if (iwitness_get_post_field_value('plan') == 'giftplanyear' || iwitness_get_post_field_value('plan') == 'freegiftcard') {
                    $id = rand(1, 1000);
